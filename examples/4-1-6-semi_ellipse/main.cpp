...
int run_a_scheme(const std::string& scheme, std::list<GnuplotActor>& list_a){
    ...
    EventSetFieldCenter esfc("exact", 
        [](Vt x, Vt y, Vt z, Vt t){
            if(y >= x - 1.0 / 6.0&& y <= x + 1.0 / 6.0){
                Vt s = 1.0 - (36.0 * (y - x) * (y - x));
                return std::sqrt(s);
            }
            else{
                return 0.0;
    ...
    auto a_exact = ToGnuplotActorSection(equ.field("exact"), _X_, 0.7);
    a_exact.title("Exact");
    a_exact.with_lines();
    a_exact.line_color_black();
    gun_section.add(a_exact);

    auto a_section = ToGnuplotActorSection(equ.field("phi"), _X_, 0.7);
