...
本文介绍 Carpio 中 ``OGrid`` 的基本实现原理。阅读重点是
``include/domain/octree/grid/ogrid.hpp``，同时结合
``onode.hpp``、``ocell.hpp``、``ogrid_uniform.hpp``、
``ogrid_nonuniform.hpp`` 和 ``morton.hpp`` 理解整个八叉树网格模块。
...
八叉树适合表达自适应网格。计算区域不需要整体加密，只需要在局部复杂区域
继续细分。这样可以在保持局部精度的同时减少节点数量。
