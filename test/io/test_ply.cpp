...
TEST(ply, write) {
	std::string filename = "test_output/w.ply";
	...
	myFile.add_properties_to_element("vertex",
	...
	myFile.add_properties_to_element("face", { "vertex_indices" },
			vertexIndicies, 3, PlyProperty::Type::UINT8);
	myFile.add_properties_to_element("face", { "texcoord" }, faceTexcoords, 6,
...
TEST(ply, read) {
	...
	for (auto e : file.get_elements()) {
		...
		for (auto p : e.properties) {
			...
			if (p.isList) {
				std::cout << " islist type " << PropertyTable[p.listType].str
						<< " count " << p.listCount;
			}
	...
	uint32_t vertexCount, normalCount, colorCount, faceCount, faceTexcoordCount,
			faceColorCount;
	vertexCount = normalCount = colorCount = faceCount = faceTexcoordCount =
			faceColorCount = 0;

	// The count returns the number of instances of the property group. The vectors
	// above will be resized into a multiple of the property group size as
	// they are "flattened"... i.e. verts = {x, y, z, x, y, z, ...}
	vertexCount = file.request_properties_from_element("vertex",
			{ "x", "y", "z" }, verts);
	std::cout << "\t request :" << verts.size() << " total vertices ("
			<< vertexCount << " properties)." << std::endl;
    ASSERT_EQ(vertexCount, 12);
	normalCount = file.request_properties_from_element("vertex", { "nx", "ny",
			"nz" }, norms);
	colorCount = file.request_properties_from_element("vertex", { "red",
	...
	faceCount = file.request_properties_from_element("face",
			{ "vertex_indices" }, faces);
	//faceTexcoordCount = file.request_properties_from_element("face", {
	//		"texcoord" }, uvCoords, 6);
	...
	//		<< std::endl;
	std::cout << "\tRead " << verts.size() << " total vertices (" << vertexCount
			<< " properties)." << std::endl;
	std::cout << "\tRead " << norms.size() << " total normals (" << normalCount
			<< " properties)." << std::endl;
	std::cout << "\tRead " << colors.size() << " total vertex colors ("
			<< colorCount << " properties)." << std::endl;
	std::cout << "\tRead " << faces.size() << " total faces (triangles) ("
