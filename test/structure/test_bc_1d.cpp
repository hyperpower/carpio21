...
void GnuplotAddGhostCenterValue(Gnuplot& gnu, double x, double y){
	    ...
	    a.style("with points ps 2 pointtype 12 lc palette");
        gnu.add(a);
}
...
TEST(field_face, dim1){
    ...
    for(auto index : f.order()){
        ...
        std::cout << f.ghost().is_boundary(index, _X_, _P_);
    }

    BoundaryConditionValue bc(1, 0.5);
	...
	gnu.set_terminal_png(FIG_PATH + "SturctureFieldFace1d_BC", 
