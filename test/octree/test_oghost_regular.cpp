...
TEST(oghost_regular, constructs_with_grid) {
    ...
    using Ghost = OGhostRegular_<Grid>;

    auto grid = std::make_shared<Grid>(3, 4);
    Ghost ghost(grid);
...
TEST(oghost_regular, classifies_root_and_descendant_ghost_nodes_2d) {
    ...
    auto grid = std::make_shared<Grid>(Point(0.0, 0.0), 1.0, 3, 4);
    Ghost ghost(grid);
    ...
    ghost_root->new_child(Node::Idx::_PP_);
    ASSERT_NE(ghost_root->child[Node::Idx::_PP_], nullptr);
    EXPECT_TRUE(ghost.is_ghost(ghost_root->child[Node::Idx::_PP_]));

    auto normal_root = grid->root_node(1, 1);
    normal_root->new_child(Node::Idx::_MM_);
    ASSERT_NE(normal_root->child[Node::Idx::_MM_], nullptr);
    EXPECT_FALSE(ghost.is_ghost(normal_root->child[Node::Idx::_MM_]));
}
...
TEST(oghost_regular, foreach_root_visits_all_ghost_roots_2d) {
    ...
    std::vector<Grid::Indices> visited;
    ghost.for_each_root([&visited, &grid](Grid::ref_Node node) {
        visited.push_back(grid->storage_1d_idx_to_indices(node.root_idx()));
    });

    EXPECT_EQ(visited.size(), 18);
    EXPECT_NE(std::find(visited.begin(), visited.end(), Grid::Indices(-1, -1)), visited.end());
    EXPECT_NE(std::find(visited.begin(), visited.end(), Grid::Indices(-1, 2)), visited.end());
    EXPECT_NE(std::find(visited.begin(), visited.end(), Grid::Indices(1, -1)), visited.end());
    EXPECT_NE(std::find(visited.begin(), visited.end(), Grid::Indices(3, 1)), visited.end());
    EXPECT_NE(std::find(visited.begin(), visited.end(), Grid::Indices(2, 4)), visited.end());
    EXPECT_EQ(std::find(visited.begin(), visited.end(), Grid::Indices(1, 1)), visited.end());

    const Ghost& const_ghost = ghost;
    St const_count = 0;
    const_ghost.for_each_root([&const_count](Grid::const_ref_Node) {
        ++const_count;
    });
...
TEST(oghost_regular, detects_z_boundaries_with_connected_neighbors_3d) {
    ...
    auto grid = std::make_shared<Grid>(2, 2, 2);
    grid->connect_neighbors();
    Ghost ghost(grid);
...
TEST(oghost_regular, detects_refined_child_boundaries_with_connected_neighbors) {
    ...
    auto grid = std::make_shared<Grid>(Point(0.0, 0.0), 1.0, 2, 2);
    grid->connect_neighbors();
    Ghost ghost(grid);
    ...
    ghost_root->new_child(Node::Idx::_MP_);

    auto normal_child = normal_root->child[Node::Idx::_MM_];
...
TEST(oghost_regular, gnuplot_ghost_roots_2d) {
    ...
    auto grid = std::make_shared<Grid>(Point(0.0, 0.0), 0.25, 3, 4);
    ...
    ASSERT_NE(left_ghost->child[Node::Idx::_PP_], nullptr);
    left_ghost->child[Node::Idx::_PP_]->new_full_children();

    bottom_ghost->new_child(Node::Idx::_PM_);
    corner_ghost->new_child(Node::Idx::_PP_);
